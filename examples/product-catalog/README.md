...
Import from the dedicated mirror repo
**[`rap2next-sample-catalog`](https://github.com/palimkarakshay/rap2next-sample-catalog)**
(standard `/src/` layout — the cloud abapGit ADT plugin does not reliably resolve
a nested starting folder inside this monorepo, which shows up as
"package could not be imported"/an empty pull):

- **BTP ABAP Environment (incl. trial):** ADT → *abapGit Repositories* view →
  Link → URL `https://github.com/palimkarakshay/rap2next-sample-catalog.git`,
  branch `main`, into your (empty) package `ZR2N_DEMO` → **Pull**.
...
The mirror is generated by `scripts/sync-sample.sh` from `abap/` here — this
